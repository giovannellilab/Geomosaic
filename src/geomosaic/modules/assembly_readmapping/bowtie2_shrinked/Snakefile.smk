...
rule run_bowtie2:
    ...
    output:
        folder=directory("{wdir}/{sample}/bowtie2_shrinked"),
        sam_file=temp("{wdir}/{sample}/bowtie2_shrinked/read_mapping.sam"),
        bam_file=temp("{wdir}/{sample}/bowtie2_shrinked/read_mapping.bam"),
    ...
    params:
        ...
        samtools_sort_user_params=( lambda x: " ".join(filter(None , yaml.safe_load(open(x, "r"))["samtools_sort"])) ) (config["USER_PARAMS"]["bowtie2_shrinked"]),
        samtools_index_user_params=( lambda x: " ".join(filter(None , yaml.safe_load(open(x, "r"))["samtools_index"])) ) (config["USER_PARAMS"]["bowtie2_shrinked"])
    shell:
        ...
        samtools view -@ {threads} {params.samtools_view_user_params} -F 4 -S -b {output.sam_file} > {output.bam_file}
