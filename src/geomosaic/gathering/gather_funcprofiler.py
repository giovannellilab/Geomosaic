...
def gather_funcprofiler(config_file,geomosaic_wdir,output_base_folder,additional_info):
    pckg = "funcprofiler"
    ...
    compose_matrix_funcprofiler(geomosaic_wdir, output_folder, samples)

def compose_matrix_funcprofiler(folder, output_folder, samples):
    ...
    for t in ['ko_profiles','prefetch_out']:
        ...
        for s in samples:
            folder_data = os.path.join(folder,s,"funcprofiler")
            flag = True
            ...
            if t == 'prefetch_out':
                ...
                raw_df.rename(columns={"match_name":pivot, "intersect_bp": "raw_count"}, inplace=True)
